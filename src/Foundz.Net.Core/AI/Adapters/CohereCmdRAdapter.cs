...
public class CohereCmdRAdapter : IProviderAdapter
{
    ...
    public object FormatTools(List<ToolDefinition> tools)
    {
        ...
        foreach (var tool in tools)
        {
            var toolDef = new
            {
                ...
                parameter_definitions = FormatParameters(tool.ParametersSchema)
            };
        ...
        try
        {
                ...
                && toolCallsProp.ValueKind == JsonValueKind.Array)
            {
                foreach (var call in toolCallsProp.EnumerateArray())
                {
                    ...
                    var parameters = call.TryGetProperty("parameters", out var paramsProp)
                        ? paramsProp
        ...
        try
        {
            ...
            else if (eventType == "tool-calls-generation")
            {
                ...
                return new AIResponseChunk(
                    ...
                    FinishReason: null,
                    IsComplete: false,
                    ToolCallDelta: json.GetProperty("tool_calls").ToString()
                );
            }
    ...
    public ModelCapabilities GetCapabilities(string modelName)
    {
        return new ModelCapabilities
        {
            ...
            SupportedLanguages = new[] { "en", "fr", "es", "de", "it", "pt", "ja", "ko", "zh", "ar" },
    ...
    private object FormatParameters(JsonElement parametersSchema)
    {
        ...
        if (!parametersSchema.TryGetProperty("properties", out var properties))
            return parameters;
